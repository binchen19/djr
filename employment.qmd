...
|-----------------------------|-----------------|---------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| `read_csv()`                | `readr`         | Read CSV files into R                            | `read_csv("data/employment.csv")`                                                                             |
| `rename()`                  | `dplyr`         | Rename columns in a data frame                    | `rename(year = Academic Year, university = University)`                                                       |
| `str_sub()`                 | `stringr`       | Extract substring from a string                  | `str_sub(year, 1, 4)`                                                                                        |
| `as.integer()`              | Base R          | Convert values to integer type                   | `as.integer(year_start)`                                                                                     |
| `mutate()`                  | `dplyr`         | Create or modify columns                         | `mutate(year_start = str_sub(year, 1, 4))`                                                                    |
| `select()`                  | `dplyr`         | Select specific columns                          | `select(year_start, university, level, employment, headcount)`                                                 |
| `glimpse()`                 | `dplyr`         | Get quick overview of data frame structure        | `glimpse(df_cleaned)`                                                                                        |
| `filter()`                  | `dplyr`         | Filter rows based on conditions                  | `filter(employment == "Unemployed")`                                                                          |
| `group_by()`                | `dplyr`         | Group data by one or more variables              | `group_by(university, year_start)`                                                                            |
| `summarise()`               | `dplyr`         | Compute summary statistics for groups            | `summarise(n = sum(headcount))`                                                                               |
| `left_join()`               | `dplyr`         | Merge datasets keeping all rows from left table   | `left_join(df_total, by = c("university", "year_start"))`                                                     |
| `anti_join()`               | `dplyr`         | Keep rows not matching another table             | `anti_join(df_total)`                                                                                        |
| `unique()`                  | Base R          | Extract unique values                            | `unique(df$University)`                                                                                      |
| `table()`                   | Base R          | Build contingency table of counts                | `table(df$Occupation)`                                                                                       |
| `mean()`                    | Base R          | Calculate arithmetic mean                        | `mean(df_percentage$percentage)`                                                                              |
| `median()`                  | Base R          | Calculate median (middle value)                  | `median(df_percentage$percentage)`                                                                            |
| `min()`                     | Base R          | Find minimum value                               | `min(df_percentage$percentage)`                                                                               |
| `max()`                     | Base R          | Find maximum value                               | `max(df_percentage$percentage)`                                                                               |
| `summary()`                 | Base R          | Generate comprehensive summary statistics        | `summary(df_percentage$percentage)`                                                                            |
| `hist()`                    | Base R          | Create histogram of numeric data                 | `hist(df_percentage$percentage)`                                                                              |
| `skim()`                    | `skimr`         | Quick data summary and exploration                | `skim(df)`                                                                                                   |
| `ggplot()`                  | `ggplot2`       | Create a new ggplot object                        | `ggplot(aes(x = university, y = percentage))`                                                                 |
| `aes()`                     | `ggplot2`       | Specify aesthetic mappings (x, y, color, fill)   | `aes(x = fct_reorder(university, percentage), y = percentage, color = university)`                             |
| `geom_col()`                | `ggplot2`       | Create a bar chart                               | `geom_col()`                                                                                                 |
| `geom_line()`               | `ggplot2`       | Create a line plot                               | `geom_line()`                                                                                                |
| `geom_point()`              | `ggplot2`       | Add points to a plot                             | `geom_point()`                                                                                               |
| `geom_text()`               | `ggplot2`       | Add text labels to a plot                        | `geom_text(aes(label = round(percentage, 1)), hjust = -0.1)`                                                  |
| `coord_flip()`              | `ggplot2`       | Flip x and y axes                                | `coord_flip()`                                                                                               |
| `scale_x_continuous()`      | `ggplot2`       | Customize continuous x-axis scale                | `scale_x_continuous(breaks = seq(2009, 2022, 1))`                                                             |
| `labs()`                    | `ggplot2`       | Add titles, labels, and captions                 | `labs(title = "...", x = "Year", y = "Percentage (%)")`                                                       |
| `theme_bw()`                | `ggplot2`       | Apply black-and-white theme                      | `theme_bw()`                                                                                                 |
| `facet_wrap()`              | `ggplot2`       | Create multiple plots based on categorical var   | `facet_wrap(~university)`                                                                                     |
| `theme()`                   | `ggplot2`       | Customize plot appearance                        | `theme(legend.position = "bottom")`                                                                            |
| `fct_reorder()`             | `forcats`       | Reorder levels of a categorical variable         | `fct_reorder(university, percentage)`                                                                         |
| `ggsave()`                  | `ggplot2`       | Save plot as image file                          | `ggsave("line_plot.png", width = 10, height = 6, dpi = 300)`                                                  |
| `grid.arrange()`            | `gridExtra`     | Arrange multiple plots in a grid                 | `grid.arrange(p1, p2, ncol = 2)`                                                                              |
| `ggplotly()`                | `plotly`        | Convert ggplot to interactive plotly plot        | `ggplotly(p)`                                                                                                 |
| `n()`                       | `dplyr`         | Get number of rows in current group               | `summarise(n = n())`                                                                                         |
