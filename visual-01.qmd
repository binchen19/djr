...
`labs()` adds communication text without changing the data or marks. Name the
title, axes, and source explicitly:
...
ggplot(gpfg_latest, aes(x = market_value_usd)) +
  ...
  labs(
    title = "Most company holdings were small relative to the largest",
...
`geom_point()` draws one point for each row using its x and y values. Here one
point represents one holding. A visible relationship does not show that one
variable caused the other. Chapter 8 will address overlapping points, skewed
values, and comparisons among regions.
