...
Now, you have a basic understanding of R/RStudio, R Project, and R Markdown. Next, we will cover some fundamental syntax and concepts in R programming.
...
::: {.callout-tip}
Use comments to explain the why behind your code, not the how or what. This presents confusion in the future, especially when revisiting complex projects or after a long time.
:::

## Naming Conventions

Object names must start with a letter and can contain letters, numbers, underscores (`_`), abd periods(`.`). We recommend using `snake_case` for multi-word object names, which means using lowercase letters and underscores to separate words (e.g., `birth_year`, `student_name`).
...
R provides a variaty of built-in functions. They are called by typing the function name followed by parentheses `()`. If the function requires arguments, you can pass them inside the parentheses.
...
Charater data type represents text or string values. Character values are enclosed in double quotes (`"`) or single quotes (`'`).
...
Data frame is a two-dimensional data structure that stores data in *rows* and *columns*. Eacb row represents an observation, and each column represents a variable.

We can build a data frame using the `data.frame()` function.

First, let's create a data frame with two columns: `name` and `age`.
